...
The existing provider architecture consisted of cloud and local adapters behind shared `AIProvider`, router, and model-bridge contracts. The application defaulted to a cloud provider and required a provider credential for the active execution path.
...
The provider was registered in `providerRouter.ts` and `modelProviders.ts`. Local Mode is now the default provider for a new session, while Anthropic and Moonshot remain available as optional providers. The remaining cloud-provider adapters were not removed or replaced.
